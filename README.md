...
Below is an idea of trying to, a bit like with BERT, add a way to be able to predict what has happened before but an experimentation has been done where a new architecture is not needed. LLAMA v2 7b has been trained with qlora on vicuna dataset but backwards in order to be able to get probabilities of what is most probable to come before a set of tokens, yellow below. If combined with some graph additions the hope is that this will result in retrospective contemplation where the LLMs are able to correct its world model.
