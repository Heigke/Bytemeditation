...
def calculate_metric(values):
    ...
    metric = 1 - entropy / math.log2(total_values)
...
def get_strace_metric():
		...
		while len(calls) == 0:
			...
			strace_output = run_strace(binary_path)
			run_awk_extract()
...
def ppo_step(model, input_ids, labels, optimizer, clip_param=0.2, value_coeff=0.5, entropy_coeff=0.01):
    ...
    reward = torch.tensor([get_strace_metric()])
...
#    print("Distributions drawn")
    ...
    ppo_loss=ppo_step(model_ppo,prompt,last_thought,optimizer_ppo)
