...
from wolfram_man import wolfram_answer
#****************SETUP**********************************************
openai.api_key = ""
model_name = 'gpt-4'  
URL = "
...
def get_wolfram_answer(q):
	q=str(q)
	chat_list= [{"role":"system","content":"Please answer with just a differential equation which is playful and simple, answer as a json object in bracket {} and nothing more. "} ,
	{"role":"user","content":q}]
	chat_list=[chat_list]
	uuid = put_question_in_prompt_queue(chat_list,None,None)
	ans_dict = get_completed_children([uuid],"wolfram")
	wolf_query = ans_dict[uuid][0][-1]["content"]
	print("WOLFRAM QUERY: "+str(wolf_query))
	if wolf_query != 0:
		wolf_result = wolfram_answer(wolf_query)
		print("WOLFRAM SUCCESS!")
	else:
		return ""
	return "Mathematical Interpretation: "+wolf_query+" "+wolf_result
...
class Node:
	...
	# Append them to self.children
		...
		question_list.insert(0,"""As an advisor who is knowledgeable, extremely provocative and very concise, you are exploring a primary question by generating an extremely concise answer and follow-up question. Keep your answer very short, maximum two sentences.""")
		chat_list = []
		...
		wolfram_answer = get_wolfram_answer(chat_list[-1]["content"])
		chat_list[-1]["content"] = "Web search: "+web_search+" Mathematical interpretation: "+wolfram_answer+" Related memory: "+related_memory+". \n Thought: "+chat_list[-1]["content"]
...
if __name__ == "__main__":
	...
	send_whatsapp.send_msg("Started tree with Question: "+question)
	best_node=tot_bfs(root_node,2,group_id_arg,int(depth))

	best_value = best_node.evaluation_score
	best_answer = best_node.state
	send_whatsapp.send_msg("Finished tree with Question: "+question+" And best thought: "+best_answer)
